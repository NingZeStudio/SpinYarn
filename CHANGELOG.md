...
> 修复 v1.1.0 Redis 缓存在常驻进程下的两个运行期问题：每次反混淆的全量 JSON 反序列化既慢（实测单次 37-54ms）又在 glibc 堆上留下高水位（JSON 小对象碎片不归还），宿主 RSS 随任务量无界爬升（线上观测 800+ MB）。

### 变更
- **序列化格式 JSON → bincode**：映射缓存以整块 buffer 编解码。解析耗时降至毫秒级；大块分配走 mmap 路径，free 时 munmap 直接归还内核，消除堆高水位驻留。解码兼容 v1.1.0 存量 JSON 键（首字节 `{` 判定），升级期间新旧格式并存可读
- **Redis 连接复用**：`Cache` 持有常驻连接（`Mutex<Option<Connection>>`），命令失败自动重连重试一次；不再每次调用新建 TCP 连接
- **Redis 侧版本高低水位**：LRU 记账集合 `spinyarn:mapping:lru`（ZSET，score=最近使用秒），缓存版本数超过高水位 10 时按最旧优先弹出并删除映射键，回落至低水位 8；`CacheStats.entries` 实化为 ZSET 计数

### 兼容性
- C ABI 与 PHP 扩展签名不变（`spinyarn_init` 第 5 参 `redis_url` 同 v1.1.0）；宿主无需改代码
...
- **长期支持（LTS）**：v1.0.0 起进入稳定维护期，破坏性变更将只进入后续大版本。
- 相对 v1.0.0-pre.2 无功能变更，仅版本号正式化。

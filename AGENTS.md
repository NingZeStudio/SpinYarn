...
- **并发限流**：`src/api/deobfuscate.rs::GATE` 全局 `Semaphore`，默认 32（`SPINYARN_MAX_CONCURRENCY` 环境变量可调）。无缓存模型下每并发请求持有一整套版本表（~30MB），限流把峰值内存钉在 N×30MB，突发流量 OOM 换成短暂排队（稳态并发约 16，平时不触发）
...
- 仅两个端点：`POST /api/v1/deobfuscate`（64MB 上限）、`GET /api/v1/health`
- 管理端点（load/unload/list/version）已在 v2 移除

## 关键约定

### 内置版本 + 透传
`src/config.rs::SUPPORTED_VERSIONS` 硬编码 1.14 ~ 1.21.11 共 43 个版本。请求版本不在列表内 → **原样透传**（`success: true`，计数为 0），不报错。
...
- 运行时加载优先级：嵌入式表 → 外部 `SPINYARN_MAPPINGS_DIR` 目录覆盖 → 都没有则透传

### 配置加载
`Config::load()` 按顺序查找：`config.toml` → `SpinYarn.toml` → `/etc/spinyarn/config.toml`，都没找到则使用默认值（`127.0.0.1:8080`）。
...
- 非堆栈行：预编译正则兜底（`src/deobfuscator/pattern.rs`），贪婪匹配天然免疫前缀冲突
...
集成测试依赖 `jq` 和 `curl`。若 8080 被占用，先 `pkill -f target/release/spinyarn`。
